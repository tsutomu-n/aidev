...
class TerrainTests(unittest.TestCase):
    ...
    def test_idempotent_and_context_gating(self):
        ...
        self.assertIn('+00:00', state['pack_built_at'])
        self.assertEqual(len(state['pack_snapshot_id']), 64)
        self.assertIsNone(state['context_built_at'])
        self.assertEqual(self.acp_calls, [])
        calls = len(self.calls)
        self.assertEqual(self.init()['indexes'], 'reused')
        self.assertEqual(len(self.calls), calls)
        self.assertEqual(provider.load_json(self.root, provider.STATE)['pack_built_at'], state['pack_built_at'])
        ...
        built_state = provider.load_json(self.root, provider.STATE)
        self.assertIn('+00:00', built_state['context_built_at'])
        self.assertEqual(len(built_state['context_snapshot_id']), 64)
        self.assertEqual(built_state['pack_built_at'], state['pack_built_at'])
        calls = len(self.calls)
    ...
    def test_dirty_staged_untracked_and_ignored(self):
        ...
        summary = provider.doctor_summary(self.root, provider.doctor(self.root))
        self.assertEqual(summary['pack']['status'], 'STALE')
        self.assertEqual(summary['next'], 'aidev terrain refresh')
