...
`aidev`を導入済みなら、**解析したいリポジトリのGit root**で実行します。aidevのソースcheckoutで実行する必要はありません。
...
ソースと導入済みCLIは別です。インストーラーは必要なファイルを平坦なreleaseディレクトリへ配置します。ソースを編集しただけでは通常利用環境のCLIは更新されません。導入・更新は上記の手順書に従ってください。
